...
const LCP_ADMIN = (() => {
  ...
  async function initOrders() {
    ...
    const { data: orders, error: ordersErr } = await LCP_DB.orders.listAll();
    ...
    function orderCard(o) {
      const itemsText = [...o.items, ...o.deals]
        .map((l) => `${l.product_name_snapshot || l.deal_name_snapshot} × ${l.quantity} (${LCP_UTIL.pkr(l.line_total)})`)
        .join(", ");
    ...
    LCP_UTIL.qsa("#order-section-chips .chip").forEach((c) => c.addEventListener("click", () => {
      LCP_UTIL.qsa("#order-section-chips .chip").forEach((x) => x.classList.remove("active"));
  ...
  async function initProducts() {
    ...
    const [{ data: products, error: prodErr }, { data: categories, error: catErr }] = await Promise.all([LCP_DB.catalog.listProducts(), LCP_DB.catalog.listCategories()]);
    if (prodErr || catErr) LCP_UTIL.toast("Unable to load products. Please refresh.", "error");
    ...
    function row(p) {
      ...
      tr.querySelector("[data-image-cell]").appendChild(
        buildImageCell(p, "products", (patch) => LCP_DB.catalog.updateProduct(p.id, patch))
      );
  ...
  async function initDeals() {
    ...
    let { data: deals, error: dealsErr } = await LCP_DB.catalog.listDeals();
    if (dealsErr) LCP_UTIL.toast("Unable to load deals: " + dealsErr, "error");
    ...
    function renderList() {
      ...
      deals.forEach((d) => {
        ...
        card.querySelector("[data-deal-image-cell]").appendChild(
          buildImageCell(d, "deals", (patch) => LCP_DB.catalog.updateDeal(d.id, patch))
        );
    ...
    function render(list) {
      ...
      wrap.innerHTML = `<table class="admin-table">
        ...
        <tbody>${list.map((o) => `<tr><td><strong>${o.order_number}</strong></td><td>${LCP_UTIL.fmtDate(o.delivered_at || o.rejected_at || o.created_at)}</td>
          ...
          <td><span class="badge ${statusBadgeClass(o.status)}">${statusLabel(o.status)}</span></td></tr>`).join("")}</tbody></table>`;
    }
    render(finished);
