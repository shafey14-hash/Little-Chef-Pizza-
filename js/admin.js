...
const LCP_ADMIN = (() => {
  ...
  async function initDashboard() {
    ...
    if (ordersErr || productsErr)
      LCP_UTIL.toast(
        "Some dashboard data couldn't be loaded. Please refresh.",
        "error",
      );
  ...
  async function initOrders() {
    ...
    if (ordersErr)
      LCP_UTIL.toast("Unable to load orders: " + ordersErr, "error");
    ...
    function orderCard(o) {
      const lineItemsHtml = [...o.items, ...o.deals]
        .map(
          (l) =>
    ...
    function priceCell(p) {
      if (p.sizes)
        return Object.entries(p.sizes)
          .map(([sz, price]) => `${sz}: ${LCP_UTIL.pkr(price)}`)
          .join(" · ");
  ...
  async function initDeals() {
    ...
    document.getElementById("deal-search").addEventListener(
      "input",
      LCP_UTIL.debounce((e) => {
        searchTerm = e.target.value;
        renderDealCards();
  ...
  async function initHistory() {
    ...
    if (histErr)
      LCP_UTIL.toast("Unable to load order history: " + histErr, "error");
