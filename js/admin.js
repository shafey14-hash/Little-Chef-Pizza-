...
const LCP_ADMIN = (() => {
  ...
  async function initOrders() {
    ...
    function render() {
      ...
      if (!rows.length) { wrap.innerHTML = `<div class="empty-state"><div class="empty-state__icon">🧾</div><h3>No orders</h3></div>`; return; }
      ...
      LCP_UTIL.qsa("[data-mark]").forEach((btn) => btn.addEventListener("click", async () => {
        ...
        const { data, error } = await LCP_DB.orders.markDelivered(btn.dataset.mark);
        if (error) { LCP_UTIL.toast(LCP_UTIL.friendlyError(error), "error"); LCP_UTIL.setLoading(btn, false); return; }
        Object.assign(orders.find((o) => o.id === data.id), data);
        LCP_UTIL.toast(`${data.order_number} marked as delivered.`, "success");
        render();
      }));
    }

    LCP_UTIL.qsa("#order-filter-chips .chip").forEach((c) => c.addEventListener("click", () => {
      LCP_UTIL.qsa("#order-filter-chips .chip").forEach((x) => x.classList.remove("active"));
  ...
  async function initProducts() {
    ...
    const [{ data: products }, { data: categories }] = await Promise.all([LCP_DB.catalog.listProducts(), LCP_DB.catalog.listCategories()]);
    const catName = Object.fromEntries(categories.map((c) => [c.id, c.name]));

    function priceCell(p) {
      if (p.sizes) {
        return Object.entries(p.sizes).map(([sz, price]) =>
          `<div class="price-edit"><span class="muted" style="width:40px;">${sz}</span>
           <input type="number" min="0" value="${price}" data-price-size="${sz}"></div>`).join("");
      }
    ...
    function row(p) {
      ...
      tr.querySelectorAll("[data-avail]").forEach((btn) => btn.addEventListener("click", async () => {
        const val = btn.dataset.avail === "true";
        await LCP_DB.catalog.updateProduct(p.id, { available: val });
        p.available = val;
        LCP_UTIL.toast(`${p.name} marked ${val ? "available" : "unavailable"}.`, "success");
        renderAll();
      }));
      tr.querySelector("[data-save]").addEventListener("click", async (e) => {
        ...
        if (p.sizes) {
          const sizes = {};
          tr.querySelectorAll("[data-price-size]").forEach((inp) => sizes[inp.dataset.priceSize] = Number(inp.value));
          patch.sizes = sizes;
        } else {
    ...
    function renderAll() {
      ...
      wrap.innerHTML = `<table class="admin-table">
        <thead><tr><th>Product</th><th>Category</th><th>Price</th><th>Availability</th><th></th></tr></thead>
  ...
  async function initDeals() {
    ...
    function renderList() {
      ...
      deals.forEach((d) => {
        ...
        card.querySelector("[data-deal-save]").addEventListener("click", async (e) => {
        ...
        card.querySelector("[data-deal-toggle]").addEventListener("click", async () => {
          const { data } = await LCP_DB.catalog.updateDeal(d.id, { available: !d.available });
          d.available = data.available;
          LCP_UTIL.toast(`${d.name} ${d.available ? "activated" : "deactivated"}.`, "success");
          renderList();
        });
    ...
    document.getElementById("new-deal-btn").addEventListener("click", async () => {
      ...
      if (!price) return LCP_UTIL.toast("Please enter a valid price.", "error");
      const { data } = await LCP_DB.catalog.createDeal({ name, description, price, items: [] });
      deals.unshift(data);
  ...
  async function initHistory() {
    ...
    function render(list) {
      ...
      if (!list.length) { wrap.innerHTML = `<div class="empty-state"><div class="empty-state__icon">🗂️</div><h3>No delivered orders yet</h3></div>`; return; }
      wrap.innerHTML = `<table class="admin-table">
        <thead><tr><th>Order #</th><th>Date</th><th>Customer</th><th>Phone</th><th>Type</th><th>Total</th></tr></thead>
        <tbody>${list.map((o) => `<tr><td><strong>${o.order_number}</strong></td><td>${LCP_UTIL.fmtDate(o.delivered_at || o.created_at)}</td>
          <td>${LCP_NAV.escapeHtml(o.customer_name)}</td><td>${o.customer_phone}</td><td>${o.order_type}</td><td>${LCP_UTIL.pkr(o.total)}</td></tr>`).join("")}</tbody></table>`;
    }
    render(delivered);
    document.getElementById("history-search").addEventListener("input", LCP_UTIL.debounce((e) => {
      const q = e.target.value.trim().toLowerCase();
      render(delivered.filter((o) => !q || o.order_number.toLowerCase().includes(q) || o.customer_name.toLowerCase().includes(q) || o.customer_phone.includes(q)));
    }, 200));
  }
