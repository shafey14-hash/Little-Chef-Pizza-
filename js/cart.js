...
async function LCP_loadCatalogCache() {
  const [p, d, c] = await Promise.all([
    ...
    LCP_DB.catalog.listDeals(),
    LCP_DB.catalog.listCategories(),
  ]);
  // Defensive: never let `undefined` propagate into the cache. If any call
  // failed (missing RLS policy, unrun SQL migration, bad config, network
  // error), `.data` won't exist — fall back to an empty array so rendering
  // code doesn't crash with "Cannot read properties of undefined", and
  // record the error so the UI can show it instead of failing silently.
  LCP_CATALOG_CACHE.products = p.data || [];
  LCP_CATALOG_CACHE.deals = d.data || [];
  LCP_CATALOG_CACHE.categories = c.data || [];
  LCP_CATALOG_CACHE.lastError = p.error || d.error || c.error || null;
  return LCP_CATALOG_CACHE.lastError;
}
