...
const LCP_DB = (() => {
  ...
  console.log("%c[LCP] db.js build: HARDCODED-MENU-v4 (products+deals hybrid, caching, defensive fetch)", "color:#d4af37; font-weight:bold; font-size:13px; background:#111; padding:4px 8px; border-radius:4px;");
  ...
  function cacheSet(key, data) {
    try { localStorage.setItem("lcp_cache_" + key, JSON.stringify({ data, ts: Date.now() })); } catch { /* storage full/unavailable — fine, just skips caching */ }
  }
  function cacheClear(key) {
    try { localStorage.removeItem("lcp_cache_" + key); } catch { /* nothing to do */ }
  }
  ...
  async function safeFetch(fn, fallback) {
    try {
      ...
      if (result && result.__timedOut) {
        console.error("Database call took too long (>%dms) — showing hardcoded content without it.", DB_TIMEOUT_MS);
        return fallback;
      }
      const { data, error } = result;
      if (error) { console.error("Database call failed (using fallback):", error); return fallback; }
  ...
  async function imageMap() {
    ...
    const rows = await safeFetch(() => sb.from("menu_item_images").select("item_id, image_url"), []);
  ...
  async function adminProducts() {
    ...
    if (cached) return cached;
    const rows = await safeFetch(() => sb.from("products").select("*").order("created_at", { ascending: false }), []);
  ...
  async function adminDeals() {
    ...
    if (cached) return cached;
    const rows = await safeFetch(() => sb.from("deals").select("*").order("created_at", { ascending: false }), []);
  ...
  const catalog = {
    ...
    async listProducts() {
      ...
      const hardcoded = LCP_SEED.products.map((p) => ({ ...p, image_url: images[p.id] || null, source: "hardcoded" }));
    ...
    async getProduct(id) {
      ...
      if (hardcoded) {
        const category = LCP_SEED.categories.find((c) => c.id === hardcoded.category_id);
        return { data: { ...hardcoded, image_url: images[id] || null, source: "hardcoded", categories: { name: category?.name } } };
      }
      const admin = (await adminProducts()).find((p) => p.id === id);
      if (admin) {
        const category = LCP_SEED.categories.find((c) => c.id === admin.category_id);
    ...
    async listDeals() {
      ...
      const hardcoded = LCP_SEED.deals.map((d) => ({ ...d, image_url: images[d.id] || null, source: "hardcoded" }));
      return { data: [...hardcoded, ...admin] };
    },
    ...
    async setMenuImage(itemId, imageUrl) {
      if (!CONFIGURED) return { error: NOT_CONFIGURED_MSG };
      const { data, error } = await sb.from("menu_item_images")
        .upsert({ item_id: itemId, image_url: imageUrl, updated_at: new Date().toISOString() })
        .select().single();
      if (error) return { error: friendlyDbError(error) };
      cacheClear("images");
      return { data };
    ...
    async removeMenuImage(itemId) {
      if (!CONFIGURED) return { error: NOT_CONFIGURED_MSG };
      const { error } = await sb.from("menu_item_images").delete().eq("item_id", itemId);
      if (error) return { error: friendlyDbError(error) };
      cacheClear("images");
      return { data: true };
    },
  ...
  const orders = {
    ...
    async listAll() {
      ...
      return error ? { error: friendlyDbError(error), data: [] } : { data: normalize(data) };
    },
