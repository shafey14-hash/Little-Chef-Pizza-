...
const LCP_DB = (() => {
  ...
  const auth = {
    ...
    async signUp({ username, password, full_name, phone, area, address }) {
      ...
      const { error } = await sb.auth.signUp({
        ...
        password,
        options: { data: { username, full_name, phone, area, address, role: "customer" } },
      });
      if (error) {
  ...
  const orders = {
    ...
    async markDelivered(orderId) {
      const { data, error } = await sb.rpc("mark_order_delivered", { p_order_id: orderId });
      return error ? { error: friendlyDbError(error) } : { data };
    },
  };

  return { auth, catalog, orders };
})();
